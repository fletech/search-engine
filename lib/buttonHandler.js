...
export const BUTTON_HANDLER = (states, type, filterType, prev, next) => {
  const HANDLERS = {
    ...
    deviceSearch() {
      ...
      fetchAPI(
        "http://localhost:1337/api/hardwares?populate=*",
        // "https://mobilexpert-server.herokuapp.com/api/hardwares?populate=*",
        setData,
        setLoadingData
      );
    ...
    filters() {
      ...
      fetchAPI(
        "https://mobilexpert-server.herokuapp.com/api/subscriptions?populate=*",
        setData,
        setLoadingData
      );
    },
